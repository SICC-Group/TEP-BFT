...
void PBFTCacheProcessor::updateCommitQueue(PBFTProposalInterface::Ptr _committedProposal)
{
    ...
    //这一段是提交到commit队列，相当于一个proposal，为后面变更状态机使用
    ...
    std::map<IndexType, ViewChangeMsgInterface::Ptr> _viewChangeCache)
{
    ...
    auto prePreparedIndex = m_config->committedProposal()->index();
    auto maxPreparedIndex = prePreparedIndex;
    if (m_maxCommittedIndex.contains(toView))
    {
        maxPreparedIndex = m_maxCommittedIndex[toView];
    }
    auto maxPrecommitIndex = prePreparedIndex;
    ...
    if (m_config->waitSealUntil() > prePreparedIndex)
    {
    ...
    for (const auto& it : _viewChangeCache)
    {
        ...
        for (const auto& proposal : viewChangeReq->preparedProposals())
        {
            ...
            if (proposal->index() < maxPreparedIndex)
            {
                continue;
            }
    ...
    // generate prepareMsg from maxPreparedIndex to  maxPrecommitIndex
    PBFTMessageList prePrepareMsgList;
    for (auto i = (maxPreparedIndex + 1); i <= maxPrecommitIndex; i++)
    {
        ...
        PBFT_LOG(INFO) << LOG_DESC("generatePrePrepareMsg,这是一个prepre生成过程，打印消息中：") << printPBFTMsgInfo(prePrepareMsg)
                       << LOG_KV("dataSize", prePrepareMsg->consensusProposal()->data().size())
                       << LOG_KV("emptyProposal", empty);
    }
    PBFT_LOG(INFO) << LOG_DESC("当前prePrepareMsgListLength是什么")
                << LOG_KV("prePrepareMsgListLength", prePrepareMsgList.size());

    return prePrepareMsgList;
}

PBFTMessageList PBFTCacheProcessor::generatePrePrepareMsg1(
    std::map<IndexType, ViewChangeMsgInterface::Ptr> _viewChangeCache)
{
    auto toView = m_config->toView();
    auto committedIndex = m_config->committedProposal()->index();
...
PBFTMessageList PBFTCacheProcessor::preCommitCachesWithoutData()
{
    ...
    {
        //遍历每一个blockNumber下的cacheConfig,然后取出对应的变量，这里取出的是prePre信息，当前版本为没有压缩的prepre
        auto prePrePareCache  = it->second->preCommitWithoutData();
        if (prePrePareCache != nullptr)
        {
            // should not handle the proposal future than the system proposal
            if (waitSealUntil > committedIndex && prePrePareCache ->index() > waitSealUntil)
            {
                it = m_caches.erase(it);
                continue;
            }
            precommitCacheList.push_back(prePrePareCache);
        }
        it++;
    }
